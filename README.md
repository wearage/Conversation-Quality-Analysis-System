...
- **AI-инструменты**:
  - Модели для анализа тональности (**Hugging Face**).
  - Извлечение тем (**DeepPavlov**).
  - Морфологический анализ (**pymorphy2**).
  - Генерация рекомендаций (**GPT-4o**).
...
- **Python 3.8+**.
- **PostgreSQL**.
- Аккаунт **OpenAI** для доступа к **GPT-4o**.
...
**Creatio** отправляет расшифровку через Webhook в **n8n**. Пример входных данных (JSON):
...
- Отправляется в **Creatio** (OData API). Пример результата:
...
Для мониторинга системы можно интегрировать с **Telegram**.